...
DEFAULT_PARAMS = {
    "max_chain_sec": 5.0,       # max AUX/PCR gap still considered continuous
...
def classify_join(a, b, params):
    ...
    if same_source and byte_adjacent:
        # Re-joining a scan-internal split. cc_break / pcr_discontinuity are
        # within-recording discontinuities (merge them); aux/pmt boundaries are
        # genuine content changes (keep them separate).
        if a.terminated_by in ("aux_recording_boundary", "pmt_change"):
            return ("split", "recording-boundary",
                    "scan split on %s" % a.terminated_by)
        if a_aux and b_aux and a_aux["date"] != b_aux["date"]:
            return ("split", "aux-date-mismatch",
                    "%s vs %s" % (a_aux["date"], b_aux["date"]))
        return ("join", "discontinuity-marker", "same-source-contiguous",
                0.95, "byte-adjacent within one source (%s)" % a.terminated_by)

    # A gap (same source) or a different source: require positive AUX agreement.
    ...
    prov = "same-source-gap" if same_source else "cross-source"
    conf = 0.85 if same_source else 0.8
    return ("join", "discontinuity-marker", prov, conf,
            "AUX continuous across %s (+%.0fs)" % (prov, elapsed))
    ...
    for start in ordered:
        ...
        while True:
            ...
            for cand in ordered:
                ...
                if best is None or el < best_elapsed:
                    best, best_elapsed = cand, el
            if best is None:
                break
            chain.append(best)
            used.add(id(best))
